...
func (m App) helpBar() string {
	...
	case m.op == OpApproving:
		if m.bulkPending > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Approving %d PRs… (%d remaining)", m.bulkTotal, m.bulkPending))
		}
		return confirmStyle.Render(fmt.Sprintf("  Approving #%d…", m.confirmNum))
	case m.op == OpClosing:
		if m.bulkPending > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Closing %d PRs… (%d remaining)", m.bulkTotal, m.bulkPending))
		}
		return confirmStyle.Render(fmt.Sprintf("  Closing #%d…", m.confirmNum))
	case m.op == OpMerging:
		if m.bulkPending > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Merging %d PRs… (%d remaining)", m.bulkTotal, m.bulkPending))
		}
		return confirmStyle.Render(fmt.Sprintf("  Merging #%d…", m.confirmNum))
	case m.op == OpConfirmClose:
		if nSel > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Close %d PRs? press ", nSel)) +
				keyStyle.Render("y") +
				confirmStyle.Render(" to confirm, ") +
				keyStyle.Render("n") +
				confirmStyle.Render(" to cancel")
		}
		return confirmStyle.Render("  Close #"+itoa(m.confirmNum)+" ") +
			confirmNameStyle.Render(trunc(m.confirmTitle, 50)) +
	...
	case m.op == OpConfirmApprove:
		if nSel > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Approve %d PRs? press ", nSel)) +
				keyStyle.Render("y") +
				confirmStyle.Render(" to confirm, ") +
				keyStyle.Render("n") +
				confirmStyle.Render(" to cancel")
		}
		return confirmStyle.Render("  Approve #"+itoa(m.confirmNum)+" ") +
			confirmNameStyle.Render(trunc(m.confirmTitle, 50)) +
			confirmStyle.Render("? press ") +
			keyStyle.Render("y") +
			confirmStyle.Render(" to confirm, ") +
	...
	case m.op == OpConfirmMerge:
		if nSel > 0 {
			return confirmStyle.Render(fmt.Sprintf("  Squash & merge %d PRs? press ", nSel)) +
				keyStyle.Render("y") +
				confirmStyle.Render(" to confirm, ") +
				keyStyle.Render("n") +
				confirmStyle.Render(" to cancel")
		}
		return confirmStyle.Render("  Squash & merge #"+itoa(m.confirmNum)+" ") +
			confirmNameStyle.Render(trunc(m.confirmTitle, 50)) +
			confirmStyle.Render("? press ") +
			keyStyle.Render("y") +
			confirmStyle.Render(" to confirm, ") +
