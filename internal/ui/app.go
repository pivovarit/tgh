...
const (
	...
	OpConfirmApprove          // showing approve confirm (y/n)
	OpConfirmClose            // showing close confirm (y/n)
	OpConfirmMerge            // showing squash-merge confirm (y/n)
	OpApproving               // approve in progress
	OpClosing                 // close in progress
...
func newWithClient(c gh.Client, owners []string) App {
	t, w := buildTable(nil, nil, nil, nil, 120)
	return App{
		client:      c,
		owners:      owners,
		loading:     true,
...
// selectedPRs returns all PRs that are currently marked, in list order.
...
// toggleSelect marks or unmarks the current cursor PR.
func (m App) toggleSelect() App {
	...
	m.table.SetRows(buildRows(filtered, m.checkStatus, m.reviewStatus, m.selected, m.widths))
	return m
}

func (m App) refreshTableRows() App {
	m.table.SetRows(buildRows(m.filteredPRs(), m.checkStatus, m.reviewStatus, m.selected, m.widths))
	return m
}

// removePR removes a PR from the list and repositions the cursor to the next
// available item, preserving the scroll position of the existing table.
...
func (m App) rebuildTable(selectedNum int) App {
	...
	m.table, m.widths = buildTable(filtered, m.checkStatus, m.reviewStatus, m.selected, m.width)
